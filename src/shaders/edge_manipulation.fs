...
uniform float edge_intensity = 0.2;
...
void main()
{
  ...
  vec2 texelEdge = texture(EDGE_TEXTURE, fragTexCoord).ba;
  vec4 texelControl = texture(CONTROL_TEXTURE, fragTexCoord);

  // EDGE MODULATION
  float ctrl_intensity = texelControl.r * 5.0;
  ...
  //dark_edge_intensity = mix(0.0, dark_edge_intensity, clamp((1.0 - (texelEdge.y * 3.0)), 0.0, 1.0));
