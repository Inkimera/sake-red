...
//uniform float uv_repeat_x = 1.0;
//uniform float uv_repeat_y = 1.0;
...
void main()
{
  ...
  vec4 texelColor = texture(ALBEDO_TEXTURE, fragTexCoord);
  ...
  diluted = clamp(diluted, 0.0, 1.0);

  // reinhard tone mapping
  //diffuse = diffuse / (diffuse + vec3(1.0));
  //specular = specular / (specular + vec3(1.0));
  //diluted = diluted / (diluted + vec3(1.0));
  // Gamma correction
  ...
  vec3 watercolor = mix(shadow, color, clamp(diffuse, 0.0, 1.0));
  ...
  //finalControl = vec4(fragColor.rgb, 1.0);
  finalControl = vec4(normalize((normal + fragPosition) * depth), 1.0);
}
