...
uniform float sdf_scale = 3.0;

layout(location = 0) out vec4 finalEdge;

vec4 rgbd(vec2 screen_uv)
{
  vec4 texelColor = texture(NORMAL_TEXTURE, screen_uv);
  ...
  return vec4(texelColor.rrr, depth);
...
void main()
{
  ...
  vec4 texelColor = texture(NORMAL_TEXTURE, fragTexCoord);
  finalEdge.rgb = vec3(edge(screen_size), texelColor.g, texelColor.b); //vec3(1.0) / (vec3(edge(screen_size)) + vec3(1.0));
	finalEdge.a = 1.0;
